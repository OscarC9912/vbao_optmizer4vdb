...
# VECTOR_TYPES = ["Other", 'topk_relative']
...
def is_vector(node):
    return node["Node Type"] in VECTOR_TYPES
...
class TreeBuilder:
    ...
    def __featurize_join(self, node):
        ...
        # print("==== Join Featurizer =====", flush=True)
    ...
    def __featurize_vector(self, node):
        ...
        # print("==== Vector Featurizer =====", flush=True)
        # print(np.concatenate((arr, self.__stats(node))), flush=True)
        # print("==== Vector Featurizer =====", flush=True)
        return np.concatenate((arr, self.__stats(node)))
    ...
    def plan_to_feature_tree(self, plan):
        ...
        if len(children) == 1:
            if 'Plan Rows' in plan:
                if int(plan['Plan Rows']) != int(self.encoding_data['topk']):
                    return self.plan_to_feature_tree(children[0])
            if 'Plan Rows' not in plan:
                print(f"Plan Rows not in plan: {plan}")
                return self.plan_to_feature_tree(children[0])
